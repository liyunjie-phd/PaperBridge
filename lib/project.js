...
function collectBuildWarnings(log) {
  const warnings = [];
  const patterns = [
    /LaTeX Warning:.+/g,
    /Class acmart Warning:.+/g,
    /Overfull \\hbox.+/g,
    /Underfull \\hbox.+/g,
    /Float too large.+/gi,
    /Too many unprocessed floats.+/gi,
    /LaTeX Error:.+/g
  ];
...
export async function compileProject(projectRoot, mainTex) {
  ...
  try {
    if (await commandAvailable("latexmk")) {
      output = await runText(
        "latexmk",
        ["-pdf", "-interaction=nonstopmode", "-halt-on-error", mainTex],
        projectRoot,
        180_000
      );
      ...
      output = await runText(
        ...
        ["--keep-logs", "--keep-intermediates", "--print", mainTex],
        projectRoot,
        240_000
      );
    } else {
  ...
  const floatLayout = success && pdf.exists ? await getFloatLayout(projectRoot, mainTex) : [];
  return {
    success: success && pdf.exists,
    pdf,
    floatLayout,
