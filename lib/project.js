...
export function collectBuildLocations(log, files = [], mainTex = "") {
  ...
  const resolveFile = (value) => {
    ...
    return normalizedFiles.find((file) => normalized === file || normalized.endsWith(`/${file}`)) || "";
  };
  const add = (file, line, message = "") => {
    const normalizedFile = resolveFile(file) || (normalizedFiles.includes(file) ? file : "");
    const normalizedLine = Math.max(1, Number(line) || 1);
    const key = `${normalizedFile}:${normalizedLine}`;
    if (!normalizedFile || seen.has(key)) return;
    seen.add(key);
    locations.push({ file: normalizedFile, line: normalizedLine, message: String(message || "").trim().slice(0, 500) });
  };

  for (const match of source.matchAll(/^(.+?\.tex):(\d+):\s*(.+)$/gm)) {
    add(match[1], match[2], match[3]);
  }
  for (const match of source.matchAll(/^l\.(\d+)\s*(.*)$/gm)) {
    ...
    add(activeFile, match[1], match[2]);
  }
  return locations.slice(0, 12);
}

function compilerFailureSummary(log) {
  const lines = String(log || "").split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
  const useful = [...lines].reverse().find((line) => (
    /(?:error|fatal|emergency|undefined control sequence|missing .* inserted|not found|failed)/i.test(line)
    && !/^For immediate help type/i.test(line)
  ));
  return (useful || "LaTeX 编译器返回了失败状态，请查看下面的编译日志和源码位置。").slice(0, 1200);
}
...
export async function compileProject(projectRoot, mainTex, { clean = false, fast = false } = {}) {
  ...
  const diagnosticLog = commandSucceeded ? log : `${output}\n${log}`.trim();
  const parsedErrors = collectBuildErrors(diagnosticLog);
  const errors = parsedErrors.length
    ? parsedErrors
    : (!commandSucceeded || !pdf.exists) ? [compilerFailureSummary(output || diagnosticLog)] : [];
  const buildSucceeded = commandSucceeded && pdf.exists && errors.length === 0;
  const previewAvailable = buildSucceeded;
  const files = errors.length
    ? await discoverTexFiles(projectRoot, mainTex).catch(() => [mainTex])
    : [];
  const locations = errors.length ? collectBuildLocations(diagnosticLog, files, mainTex) : [];
  const floatLayout = previewAvailable ? await getFloatLayout(projectRoot, mainTex) : [];
  return {
    success: buildSucceeded,
    previewAvailable,
    recoverable: !buildSucceeded && (previousPdf.exists || pdf.exists),
    pdf,
    floatLayout,
    warnings: collectBuildWarnings(diagnosticLog),
    errors,
    locations,
    engine,
    mode,
    log: diagnosticLog.split(/\r?\n/).slice(-240).join("\n")
  };
}
...
// When a project was imported from a ZIP/local folder, its freshly initialized
// Git history is unrelated to an already-populated remote repository. Merge
// the histories instead of rejecting the push. For paths present on both
// sides, keep the current local working copy; remote-only files remain in the
// repository and are not duplicated.
async function mergeUnrelatedRemoteKeepingLocal(projectRoot, remoteRef, remoteLabel = "远端仓库") {
  let mergeStarted = false;
  try {
    await runGit(["merge", "--allow-unrelated-histories", "--no-edit", "--no-commit", remoteRef], projectRoot, 120_000, "");
    mergeStarted = true;
  } catch (originalError) {
    mergeStarted = true;
    const conflictFiles = splitNullList(await runGit(["diff", "--name-only", "--diff-filter=U", "-z"], projectRoot, 20_000, "").catch(() => ""));
    if (!conflictFiles.length) {
      await runGit(["merge", "--abort"], projectRoot, 20_000, "").catch(() => {});
      throw originalError;
    }
    for (const file of conflictFiles) {
      await checkoutConflictSide(projectRoot, "--ours", file);
    }
  }
  if (!mergeStarted) return { merged: false };
  const mergeHead = await runGit(["rev-parse", "--verify", "MERGE_HEAD"], projectRoot, 20_000, "")
    .then(() => true).catch(() => false);
  if (mergeHead) {
    await runGit(["commit", "-m", `Merge ${remoteLabel} history, keeping current local files`], projectRoot, 60_000, "");
  }
  return { merged: true };
}
...
export async function pushGitRepository(projectRoot, message, options = {}) {
  ...
  if (hasRemoteBranch) {
    const related = await runGit(["merge-base", "HEAD", remoteRef], projectRoot, 20_000, "")
      .then(() => true).catch(() => false);
    if (!related) {
      await mergeUnrelatedRemoteKeepingLocal(projectRoot, remoteRef, status.remoteLabel || "远端仓库");
    }
    const remoteOnly = Number(await runGit(["rev-list", "--count", `HEAD..${remoteRef}`], projectRoot, 20_000, ""));
    if (related && remoteOnly > 0) {
      await mergeRemoteKeepingNewest(
        projectRoot,
        "push",
        remoteRef,
