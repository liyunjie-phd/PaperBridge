...
export function parseMathBlocks(content, file = "document.tex") {
  ...
  function findDisplayMathEnd(startIndex, marker, openIndex) {
    ...
    const sameLineClose = markerIndex(lines[startIndex], closeMarker, openIndex + marker.length);
    if (sameLineClose >= 0) return startIndex;
    for (let index = startIndex + 1; index < lines.length; index += 1) {
      if (lines[index].trimStart().startsWith("%")) continue;
      if (markerIndex(lines[index], closeMarker) >= 0) return index;
      if (hasDocumentBoundary && /\\end\s*\{document\}/.test(lines[index])) return index - 1;
    }
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const bracketMath = markerIndex(line, "\\[");
    if (bracketMath >= 0) {
    ...
    const dollarMath = markerIndex(line, "$$");
    if (dollarMath >= 0) {
...
export function replaceTableBlockRows(document, block, rows) {
  ...
  if (nextRows.length !== block.rows.length) {
    const error = new Error("The edited table row count no longer matches the TeX table.");
  ...
  for (let index = block.rows.length - 1; index >= 0; index -= 1) {
    ...
    const incoming = Array.isArray(nextRows[index]?.cells) ? nextRows[index].cells : nextRows[index];
    if (!Array.isArray(incoming) || incoming.length !== row.cells.length) {
      const error = new Error("The edited table column count no longer matches the TeX table.");
      error.code = "TABLE_SHAPE_CHANGED";
      throw error;
    }
    const indent = (document.lines[row.startLine - 1].match(/^\s*/) || [""])[0];
    const source = `${indent}${incoming.map(escapeLatexTableCell).join(" & ")} \\\\`;
    nextLines.splice(row.startLine - 1, row.endLine - row.startLine + 1, source);
  }
