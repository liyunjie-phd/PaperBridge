...
async function requestJson(url, options, policy = {}) {
  const timeoutMs = Math.min(180_000, Math.max(1_000, Number(policy.timeoutMs) || 120_000));
  const maxAttempts = Math.min(3, Math.max(1, Number(policy.maxAttempts) || 3));
  let lastError;
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      if (error.name === "TimeoutError" || error.name === "AbortError") {
        lastError = new Error(`AI 接口在 ${Math.round(timeoutMs / 1000)} 秒内没有响应，请检查网络或稍后重试。`);
        lastError.status = 504;
        lastError.code = "AI_PROVIDER_TIMEOUT";
      } else {
        lastError = error;
      }
      if (error.status && ![408, 429].includes(error.status) && error.status < 500) throw error;
      if (attempt < maxAttempts) await new Promise((resolve) => setTimeout(resolve, 750 * attempt));
    }
  }
