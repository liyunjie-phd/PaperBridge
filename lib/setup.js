...
export async function createNewProject(name, projectsRoot) {
  const projectRoot = await uniqueDestination(projectsRoot, String(name || "new-paper").trim() || "new-paper");
...
export async function listMainTexCandidates(projectRoot) {
  ...
  const eligible = candidates.length ? candidates : files;
  const shallowestDepth = Math.min(...eligible.map((file) => file.split("/").length));
  return eligible.filter((file) => file.split("/").length === shallowestDepth).sort((left, right) => {
...
export async function detectMainTex(projectRoot) {
  const files = await collectTexFiles(projectRoot);
  if (!files.length) {
    const mainTex = path.join(projectRoot, "main.tex");
    const initialSource = [
    ...
    await fs.writeFile(mainTex, initialSource, { encoding: "utf8", flag: "wx" }).catch((error) => {
      if (error.code !== "EEXIST") throw error;
    });
    return "main.tex";
  }
  return (await listMainTexCandidates(projectRoot))[0];
}
