...
test("paragraph API retries when a new paragraph response is still Chinese", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperbridge-new-paragraph-retry-"));
  const projectRoot = path.join(root, "project");
  let providerServer;
  try {
    await fs.mkdir(projectRoot);
    ...
    const chineseDraft = "\u8fd9\u662f\u4e00\u4e2a\u9700\u8981\u65b0\u589e\u7684\u4e2d\u6587\u6b63\u6587\u6bb5\u843d\uff0c\u7528\u4e8e\u9a8c\u8bc1\u7cfb\u7edf\u4e0d\u4f1a\u628a\u4e2d\u6587\u539f\u6837\u5199\u5165 TeX\u3002";
    const insertedEnglish = "The inserted academic paragraph verifies that PaperBridge retries incomplete model outputs before writing safe English prose into the manuscript.";
    let callCount = 0;
    providerServer = http.createServer(async (request, response) => {
      for await (const _chunk of request) {
        // Consume the request before responding so repeated calls can reuse the connection.
      ...
      response.writeHead(200, { "Content-Type": "application/json" });
      response.end(JSON.stringify({
        choices: [{ message: { content: callCount === 1 ? chineseDraft : insertedEnglish } }]
      }));
    });
    await new Promise((resolve, reject) => {
      providerServer.once("error", reject);
    ...
    assert.equal(added.document.segments.length, 3);
    assert.equal(added.document.segments[1].chinese, chineseDraft);
    assert.equal(added.document.segments[1].english, insertedEnglish);
    const sourceText = await fs.readFile(path.join(projectRoot, "main.tex"), "utf8");
    assert.equal(sourceText.includes(chineseDraft), false);
    assert.equal(sourceText.includes(insertedEnglish), true);
  } finally {
    await stopServer();
    if (providerServer) await new Promise((resolve) => providerServer.close(resolve));
    const relative = path.relative(os.tmpdir(), root);
...
test("paragraph API inserts an English draft without calling the AI provider", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperbridge-new-paragraph-english-"));
  const projectRoot = path.join(root, "project");
  try {
    await fs.mkdir(projectRoot);
    await fs.writeFile(
    ...
    const document = await request("/api/document?file=main.tex");
    const englishDraft = "This directly inserted English paragraph contains enough academic prose for manuscript editing without contacting the model.";
    const added = await post("/api/segment/add", {
      file: "main.tex",
      index: 0,
      sourceHash: document.segments[0].sourceHash,
      chinese: englishDraft,
      position: "after"
    });

    assert.equal(added.document.segments.length, 3);
    assert.equal(added.document.segments[1].chinese, englishDraft);
    assert.equal(added.document.segments[1].english, englishDraft);
    const sourceText = await fs.readFile(path.join(projectRoot, "main.tex"), "utf8");
    assert.equal(sourceText.includes(englishDraft), true);
  } finally {
    await stopServer();
    const relative = path.relative(os.tmpdir(), root);
    assert.ok(relative && !relative.startsWith("..") && !path.isAbsolute(relative));
...
test("stale Chinese autosave is ignored after the English paragraph changes", async () => {
  ...
  try {
    ...
    const savedEnglish = await post("/api/segment/english", {
      file: "main.tex",
    ...
    assert.equal(savedEnglish.build, null);
